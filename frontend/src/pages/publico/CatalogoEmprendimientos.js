...
function CatalogoEmprendimientos() {
  ...
  const filtrados = useMemo(
    () => items.filter((e) => !parroquia || e.parroquia === parroquia),
    [items, parroquia]
  );
    ...
    <div className="mx-auto max-w-7xl px-4 py-10">
      ...
      {!cargando && !error && filtrados.length === 0 && (
        <div className="rounded-2xl bg-slate-50 py-16 text-center ring-1 ring-slate-200">
          ...
          <p className="mt-1 text-sm text-slate-500">Aún no se han publicado o el filtro no coincide.</p>
        </div>
      )}

      {!cargando && !error && filtrados.length > 0 && (
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {filtrados.map((e) => <TarjetaEmprendimiento key={e.id} emprendimiento={e} />)}
        </div>
      )}
    </div>
  );
